...
def pet_detail(request, pet_id):
    ...
    booking = Booking.objects.filter(pet=pet).order_by('-booking_date').first()
    customer_address = booking.address if booking else None
...
def book_pet(request, pet_id):
    ...
    if pet.category == free_adoption_category:
        # Directly confirm booking for free adoption
        booking = Booking.objects.create(
            pet=pet,
            user=request.user,
            address=None,  # No address needed for free adoption
            payment_status=True,  # Payment status is true since no payment is required
            amount_paid=0  # No payment made
        )
        pet.is_available = False
        pet.save()
        messages.success(request, "Booking confirmed! This pet is available for free adoption.")
        return redirect('booking_success')  # Redirect to a success page

    amount_to_pay = pet.pet_price * 0.20  # 20% of the pet price
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            messages.error(request, "There was an error creating the payment order. Please try again.")
